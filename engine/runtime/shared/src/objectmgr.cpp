...
ObjectMgr::ObjectMgr()
    : m_ObjectBankNormal(32, OBJECT_PREALLOCATIONS), m_ObjectBankModel(32, MODEL_PREALLOCATIONS),
      m_ObjectBankWorldModel(32, OBJECT_PREALLOCATIONS), m_ObjectBankSprite(32, SPRITE_PREALLOCATIONS),
      m_ObjectBankLight(32, OBJECT_PREALLOCATIONS), m_ObjectBankCamera(32, OBJECT_PREALLOCATIONS),
      m_ObjectBankParticleSystem(32, OBJECT_PREALLOCATIONS), m_ObjectBankPolyGrid(32, OBJECT_PREALLOCATIONS),
      m_ObjectBankLineSystem(32, OBJECT_PREALLOCATIONS), m_ObjectBankContainer(32, OBJECT_PREALLOCATIONS),
      m_ObjectBankCanvas(32, OBJECT_PREALLOCATIONS), m_ObjectBankVolumeEffect(32, OBJECT_PREALLOCATIONS)
...
uint32 ObjectMgr::GetFrameCode() { return m_CurFrameCode; }
...
void LTObject::SetupTransform(LTMatrix &mat) { gr_SetupTransformation(&m_Pos, &m_Rotation, &m_Scale, &mat); }
...
WorldModelInstance::WorldModelInstance() : LTObject(OT_WORLDMODEL) { Clear(); }

WorldModelInstance::WorldModelInstance(char objectType) : LTObject(objectType) { Clear(); }

WorldModelInstance::~WorldModelInstance() {}

void WorldModelInstance::Init(ObjectMgr *pMgr, ObjectCreateStruct *pStruct) { LTObject::Init(pMgr, pStruct); }
...
void ModelInstance::BindModelDB(Model *pNewModel) {
  ...
  if (pNewModel != GetModelDB()) {
    ...
    DEBUG_MODEL_REZ(("model-rez: model-instance(%s) bind %s", (GetCSType() ? "s" : "c"), pNewModel->GetFilename()));
...
void ModelInstance::UnbindModelDB() {
  ...
  // delete if necessary.
  if (pModel) {
    DEBUG_MODEL_REZ(("model-rez: model-instance(%s) unbind %s", (GetCSType() ? "s" : "c"), pModel->GetFilename()));
...
uint32 ModelInstance::NodeGetRootIndex() { return GetModelDB()->GetRootNode()->m_NodeIndex; }
...
bool ModelInstance::AddChildModelDB(Model *pNewChildModel) {
  ...
  if (pParentModel) {
    ...
    if (pParentModel->AddChildModel(pNewChildModel, const_cast<char *>(filename), err_str, sizeof(err_str))) {
      DEBUG_MODEL_REZ(
          ("model-rez: add-childmodel %s -> %s ", pNewChildModel->GetFilename(), pParentModel->GetFilename()));
      return true;
    } else {
      DEBUG_MODEL_REZ(("model-rez: add-childmodel warning: %s %s -> %s", err_str, pNewChildModel->GetFilename(),
                       pParentModel->GetFilename()));
...
void ModelInstance::EnableTransformCache() {
  ...
  LT_MEM_TRACK_ALLOC(m_CachedTransforms = new LTMatrix[nNumNodes], LT_MEM_TYPE_OBJECT);

  LT_MEM_TRACK_ALLOC(m_CachedTransformInfo = new SCachedTransformInfo[nNumNodes], LT_MEM_TYPE_OBJECT);

  // only allocate rendering transforms if on the client.
  if (GetCSType() == ClientType) {
    LT_MEM_TRACK_ALLOC(m_RenderingTransforms = new DDMatrix[nNumNodes], LT_MEM_TYPE_OBJECT);
  }
...
bool ModelInstance::UpdateCachedTransformsWithPath() {
  ...
  if (!tMaker.SetupTransformsWithPath()) {
    dsi_ConsolePrint("ModelInstance::UpdateCachedTransforms failed for %s. Possible missing weightset(s)",
                     GetModelDB()->GetFilename());
    return false;
  }
...
bool ModelInstance::SetRenderStyle(uint32 iIndex, CRenderStyle *pRenderStyle) {
  ...
  pRenderStyle
      ->IncRefCount(); // Keep our ref counts up to date (the old one can get free'd this way if no-one is using it)...
  if (m_pRenderStyles[iIndex])
    renderstyles->FreeRenderStyle(m_pRenderStyles[iIndex]);
...
void ModelInstance::UpdateCollisionObjects(ModelOBB *user_obbs) {
  ...
  for (obb_cnt = 0; obb_cnt < num_obb; obb_cnt++) {
    ...
    tmp_mat.GetBasisVectors(&user_obbs[obb_cnt].m_Basis[0], &user_obbs[obb_cnt].m_Basis[1],
...
void ModelInstance::UpdateCollisionObject(uint32 obb_index, ModelOBB &user_obb) {
  ...
  // original obb xform * mat = new transform ;
  tmp_mat = xform_mat * obb_mat;
  tmp_mat.GetBasisVectors(&user_obb.m_Basis[0], &user_obb.m_Basis[1], &user_obb.m_Basis[2]);
...
ContainerInstance::ContainerInstance() : WorldModelInstance(OT_CONTAINER) { m_ContainerCode = 0xFFFF; }
...
LTVolumeEffect::LTVolumeEffect() : LTObject(OT_VOLUMEEFFECT) {
...
void LTVolumeEffect::Init(ObjectMgr *pMgr, ObjectCreateStruct *pStruct) { LTObject::Init(pMgr, pStruct); }
...
LTRESULT om_Init(ObjectMgr *pMgr, LTBOOL bClient) {
  ...
  LT_MEM_TRACK_ALLOC(sb_Init2(&pMgr->m_TrackerBank, sizeof(LTAnimTracker), TRACKER_CACHE, TRACKER_START_SIZE),
                     LT_MEM_TYPE_MISC);
...
LTRESULT om_CreateAttachment(ObjectMgr *pMgr, LTObject *pParent, uint16 nChildID, int iSocket, LTVector *pOffset,
                             LTRotation *pRotationOffset, Attachment **ppAttachment) {
