...
const char* GetVertexShaderSource()
{
	return
		...
		"    if (g_FogParams.z > 0.5f)\n"
		"    {\n"
		"        float viewZ = mul(float4(input.Pos, 1.0f), g_View).z;\n"
		"        float fogRange = max(g_FogParams.y - g_FogParams.x, 0.0001f);\n"
		"        fogFactor = saturate((viewZ - g_FogParams.x) / fogRange);\n"
		"    }\n"
...
const char* GetPixelShaderSourceTextured()
{
	return
		"cbuffer DrawPrimConstants\n"
		"{\n"
		...
		"    float FogFactor : TEXCOORD1;\n"
		"};\n"
		"float4 ApplyColorOp(float4 color, float4 texColor)\n"
		"{\n"
		"    if (g_ColorOp == 0)\n"
		"        return color;\n"
		"    if (g_ColorOp == 1)\n"
		...
		"{\n"
		"    const float refValue = 0.5f;\n"
		"    if (g_AlphaTestMode == 0)\n"
		"        return true;\n"
		"    if (g_AlphaTestMode == 1)\n"
		"        return alpha < refValue;\n"
		"    if (g_AlphaTestMode == 2)\n"
		"        return alpha <= refValue;\n"
		"    if (g_AlphaTestMode == 3)\n"
		"        return alpha > refValue;\n"
		"    if (g_AlphaTestMode == 4)\n"
		"        return alpha >= refValue;\n"
		"    if (g_AlphaTestMode == 5)\n"
		"        return alpha == refValue;\n"
		"    if (g_AlphaTestMode == 6)\n"
		"        return alpha != refValue;\n"
		"    return true;\n"
		"}\n"
		...
		"    float4 color = ApplyColorOp(input.Color, texColor);\n"
		"    if (!AlphaTest(color.a))\n"
		"        discard;\n"
		"    if (g_FogParams.z > 0.5f)\n"
		"        color.rgb = lerp(color.rgb, g_FogColor.rgb, input.FogFactor);\n"
		"    return color;\n"
		"}\n";
}

const char* GetPixelShaderSourceSolid()
{
	return
		"cbuffer DrawPrimConstants\n"
		"{\n"
