...
bool AlphaTestFail(float alpha, float ref_alpha)
{
...
{
    if (AlphaTestFail(input.color.a, 1.0f))
    {
        discard;
    }

    float3 color_linear = ToLinear(input.color.rgb);
    float4 fogged = ApplyFogLinear(float4(color_linear, input.color.a), input.view_pos);
