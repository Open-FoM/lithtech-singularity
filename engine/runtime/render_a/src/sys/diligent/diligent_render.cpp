...
const char* diligent_get_world_pixel_shader_volume_effect_source()
{
	return
		"Texture2D g_Texture0;\n"
		"SamplerState g_Texture0_sampler;\n"
		...
		"float4 PSMain(PSInput input) : SV_TARGET\n"
		"{\n"
		"    float4 texel = g_Texture0.Sample(g_Texture0_sampler, input.UV0) * input.Color;\n"
		"    if (texel.a <= 0.0f)\n"
		"    {\n"
		"        discard;\n"
		"    }\n"
		"    float3 fogged = ApplyFog(texel.rgb, input.WorldPos);\n"
		"    return float4(fogged, texel.a);\n"
		"}\n";
}
...
const char* diligent_get_world_pixel_shader_shadow_project_source()
{
	return
		...
		"float4 PSMain(PSInput input) : SV_TARGET\n"
		"{\n"
		"    float4 proj = mul(float4(input.WorldPos, 1.0f), g_WorldToShadow);\n"
		"    if (proj.w <= 0.0f)\n"
		"    {\n"
		...
		"        return float4(1.0f, 1.0f, 1.0f, 1.0f);\n"
		"    }\n"
		"    float4 shadow = g_ShadowTexture.Sample(g_ShadowTexture_sampler, uv);\n"
		"    float fade = 1.0f;\n"
		"    if (g_ProjCenter.w > 0.5f)\n"
		"    {\n"
		"        float depth = dot(input.WorldPos - g_ProjCenter.xyz, g_LightDir.xyz);\n"
		"        float maxDepth = max(g_LightDir.w, 0.001f);\n"
		"        fade = saturate(1.0f - depth / maxDepth);\n"
		"    }\n"
		"    float3 color = lerp(float3(1.0f, 1.0f, 1.0f), shadow.rgb, fade);\n"
		"    return float4(color, 1.0f);\n"
		"}\n";
}
