...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    float alpha = base_tex.a * dual_tex.a * input.color.a;
    ...
    if (dynamic_mode)
    {
        // Dynamic mode: dual textures illuminated by sun only
    ...
    {
        // Baked mode: dual textures * vertex_color + additive sun
        float3 vertex_color = lerp(1.0f.xxx, input.color.rgb, g_WorldParams.x);
        color_linear = ToLinear(base_tex.rgb) * ToLinear(dual_tex.rgb) * vertex_color + sun_light;
    }
