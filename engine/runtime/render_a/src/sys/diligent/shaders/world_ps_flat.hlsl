...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    float3 view_dir = normalize(g_CameraPos.xyz - input.world_pos);
    if (dot(normal, view_dir) < 0.0f)
    {
        normal = -normal;
    }

    float intensity = saturate(dot(normal, view_dir));
    float3 color_linear = intensity * 0.8f + 0.2f;
    color_linear = ApplySunLight(color_linear, normal);
    float4 fogged = ApplyFogLinear(float4(color_linear, 1.0f), input.world_pos);
