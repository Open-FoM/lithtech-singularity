...
namespace
{
constexpr uint32 kCharSpacing = 2;
const char kDefaultFontFile[] = "OpenSans-Regular.ttf";
...
std::string WideToUtf8(const wchar_t* text)
{
	...
	size_t needed = wcsrtombs(nullptr, &src, 0, &state);
	if (needed == static_cast<size_t>(-1))
	{
		std::string fallback;
		for (const wchar_t* p = text; *p; ++p)
		{
			fallback.push_back(static_cast<char>(*p));
		}
		return fallback;
	}
	...
	src = text;
	state = std::mbstate_t{};
	wcsrtombs(out.data(), &src, out.size(), &state);
	return out;
}
...
bool LooksLikePath(const std::string& value)
{
	...
	return EndsWithCaseInsensitive(value, ".ttf")
		|| EndsWithCaseInsensitive(value, ".otf")
		|| EndsWithCaseInsensitive(value, ".ttc");
}

std::string ResolveFontPath(const CFontInfo& font_info)
{
	std::string typeface = WideToUtf8(font_info.m_szTypeface);
	if (!typeface.empty())
	{
		if (LooksLikePath(typeface))
		{
			return typeface;
		}

		std::string lookup = typeface;
		ToLowerAscii(lookup);
		if (const char* registered = CCustomFontFileMgr::GetSingleton().FindFontFileByFamily(lookup.c_str()))
		{
			return registered;
		}
	}

	return kDefaultFontFile;
}

TTF_Font* OpenFontForInfo(const CFontInfo& font_info)
{
	if (!EnsureTtfInit())
	{
		return nullptr;
	...
	std::string path = ResolveFontPath(font_info);
	if (path.empty())
	{
		return nullptr;
	}

	TTF_Font* font = TTF_OpenFont(path.c_str(), static_cast<int>(font_info.m_nHeight));
	if (!font)
	{
		DEBUG_PRINT(1, ("TTF_OpenFont failed for '%s': %s", path.c_str(), SDL_GetError()));
		return nullptr;
	}

	int style = TTF_STYLE_NORMAL;
	if (font_info.IsStyleSet(CFontInfo::kStyle_Bold))
	{
		style |= TTF_STYLE_BOLD;
	}
	if (font_info.IsStyleSet(CFontInfo::kStyle_Italic))
	{
...
//given a string and a buffer, this will fill the buffer with all the unique letters in the
//string
void GetUniqueCharList(const wchar_t* pszString, wchar_t* pszBuffer, uint32 nBufferSize)
{
	if (nBufferSize == 0)
	{
		return;
...
bool FillGlyphDimensions(TTF_Font* font, int ascent, CTextureStringGlyph* pGlyphs, uint32 nNumGlyphs)
{
	if (!font)
	{
		return false;
	...
	const Uint32 fallback_char = static_cast<Uint32>('?');

	for (uint32 nCurrGlyph = 0; nCurrGlyph < nNumGlyphs; ++nCurrGlyph)
	{
		CTextureStringGlyph& glyph = pGlyphs[nCurrGlyph];
		Uint32 ch = static_cast<Uint32>(glyph.m_cGlyph);
		if (!TTF_FontHasGlyph(font, ch))
		{
			ch = fallback_char;
		}
		...
		glyph.m_nTotalWidth = static_cast<uint32>(advance);
		glyph.m_rBlackBox.Left() = minx;
		glyph.m_rBlackBox.Top() = ascent - maxy;
		glyph.m_rBlackBox.Right() = glyph.m_rBlackBox.Left() + (maxx - minx);
		glyph.m_rBlackBox.Bottom() = glyph.m_rBlackBox.Top() + (maxy - miny);
	}
...
bool GetTextureSizeFromCharSizes(const CTextureStringGlyph* pGlyphs, uint32 nNumGlyphs, uint32 nMaxGlyphWidth,
	uint32 nMaxGlyphHeight, uint32 nCharSpacing, SDL_Point& sizeTexture)
{
	sizeTexture.x = sizeTexture.y = 0;
	...
	if (nMaxXOffset >= MAXIMUM_TEXTURE_SIZE || nRawHeight >= MAXIMUM_TEXTURE_SIZE)
	{
		return false;
	}

	sizeTexture.x = static_cast<int>(GetTextureSize(nMaxXOffset));
	sizeTexture.y = static_cast<int>(GetTextureSize(nRawHeight));
	return true;
}

void CopyGlyphSurfaceToPixelData(SDL_Surface* surface, const LTRect2n& rectDest, uint8* pPixelData, uint32 pixelPitch)
{
	if (!surface || !pPixelData)
	{
		return;
	}

	SDL_Surface* glyph_surface = surface;
	if (glyph_surface->format != SDL_PIXELFORMAT_RGBA32)
	{
		SDL_Surface* converted = SDL_ConvertSurface(glyph_surface, SDL_PIXELFORMAT_RGBA32);
		if (converted)
		{
	...
	const int copy_width = LTMIN(rectDest.GetWidth(), static_cast<int>(glyph_surface->w));
	const int copy_height = LTMIN(rectDest.GetHeight(), static_cast<int>(glyph_surface->h));

	for (int y = 0; y < copy_height; ++y)
	{
		const uint8* src_row = static_cast<const uint8*>(glyph_surface->pixels) + y * glyph_surface->pitch;
		const uint32* src_pixels = reinterpret_cast<const uint32*>(src_row);

		uint16* dest_row = reinterpret_cast<uint16*>(
			pPixelData + ((y + rectDest.Top()) * pixelPitch) + (rectDest.Left() * 2));
		...
		for (int x = 0; x < copy_width; ++x)
		{
			...
			Uint8 b = 0;
			Uint8 a = 0;
			SDL_GetRGBA(src_pixels[x], format_details, nullptr, &r, &g, &b, &a);

			const uint16 alpha = static_cast<uint16>((a * 15) / 255);
			dest_row[x] = static_cast<uint16>((dest_row[x] & 0x0FFF) | ((alpha & 0x0F) << 12));
		}
	}
...
bool CreateGlyphTexture(CTextureStringGlyph* pGlyphs, uint32 nNumGlyphs, TTF_Font* font, HTEXTURE& hTexture)
{
	uint32 nMaxWidth = 0;
	uint32 nMaxHeight = 0;
	...
	SDL_Point sizeTexture{};
	if (!GetTextureSizeFromCharSizes(pGlyphs, nNumGlyphs, nMaxWidth, nMaxHeight, kCharSpacing, sizeTexture))
	{
		return false;
	}

	uint32 nPitch = (((16 * sizeTexture.x + 7) / 8 + 3) & ~3);
	const int pixel_data_size = static_cast<int>(nPitch * sizeTexture.y);

	uint8* pImageData = nullptr;
	LT_MEM_TRACK_ALLOC(pImageData = new uint8[pixel_data_size], LT_MEM_TYPE_UI);
	if (!pImageData)
	{
		DEBUG_PRINT(1, ("CreateGlyphTexture: Failed to allocate pixeldata."));
		return false;
	...
	uint16* pData = reinterpret_cast<uint16*>(pImageData);
	uint16* pPixelDataEnd = reinterpret_cast<uint16*>(pImageData + pixel_data_size);
	while (pData < pPixelDataEnd)
	{
		*pData = 0x0FFF;
		++pData;
	...
	for (uint32 nGlyph = 0; nGlyph < nNumGlyphs; ++nGlyph)
	{
		...
		const int nCharWidthWithSpacing = glyph.m_rBlackBox.GetWidth() + kCharSpacing;
		const int nCharRightSide = sizeOffset.x + nCharWidthWithSpacing;
		if (nCharRightSide >= sizeTexture.x)
		{
			sizeOffset.x = 0;
			sizeOffset.y += nMaxHeight + kCharSpacing;
		}
		...
		LTRect2n rCopyTo;
		rCopyTo.Left() = sizeOffset.x + (kCharSpacing / 2);
		rCopyTo.Top() = sizeOffset.y + (kCharSpacing / 2);
		rCopyTo.Right() = rCopyTo.Left() + glyph.m_rBlackBox.GetWidth();
		rCopyTo.Bottom() = rCopyTo.Top() + glyph.m_rBlackBox.GetHeight();

		if (surface)
		{
			CopyGlyphSurfaceToPixelData(surface, rCopyTo, pImageData, nPitch);
			SDL_DestroySurface(surface);
		}

		glyph.m_fU = static_cast<float>(rCopyTo.Left() + 0.5f) / static_cast<float>(sizeTexture.x);
		glyph.m_fV = static_cast<float>(rCopyTo.Top() + 0.5f) / static_cast<float>(sizeTexture.y);
		glyph.m_fTexWidth = rCopyTo.GetWidth() / static_cast<float>(sizeTexture.x);
		glyph.m_fTexHeight = rCopyTo.GetHeight() / static_cast<float>(sizeTexture.y);
	...
	if (bSuccess)
	{
		g_pILTTextureMgr->CreateTextureFromData(
			...
			TEXTUREFLAG_PREFER16BIT | TEXTUREFLAG_PREFER4444,
			pImageData,
			static_cast<uint32>(sizeTexture.x),
			static_cast<uint32>(sizeTexture.y));
...
bool CTextureStringImage::CreateBitmapFont(const wchar_t* pszString, const CFontInfo& Font)
{
	...
	TTF_Font* font = OpenFontForInfo(Font);
	if (!font)
	{
		return false;
	}
	...
	if (!FillGlyphDimensions(font, ascent, m_pGlyphList, m_nNumGlyphs))
	{
		TTF_CloseFont(font);
		FreeData();
		return false;
	...
	if (!CreateGlyphTexture(m_pGlyphList, m_nNumGlyphs, font, m_hTexture))
	{
		TTF_CloseFont(font);
		FreeData();
		return false;
	...
	m_nRowHeight = static_cast<uint32>(TTF_GetFontHeight(font));
